...
enum DocumentKind {
  ...
  // --- Company documents, uploaded during registration ---
  //
  // Kept as distinct kinds rather than one "company document" bucket because
  // verification treats them differently: the registration certificate is
  // checked against the name and number given, a licence is checked for expiry,
  // and photos are shown to candidates rather than verified at all.

  /// Business registration / incorporation certificate. The one hard
  /// requirement — everything else sharpens the check.
  companyRegistration,

  /// Trade or agency licence, where the work needs one.
  licence,

  /// Tax or GST registration.
  taxCertificate,

  /// Photos of the office or site. These are the only company upload a
  /// candidate ever sees.
  companyPhoto;
  ...
  String get label => switch (this) {
        ...
        DocumentKind.companyRegistration => 'Business registration',
        DocumentKind.licence => 'Trade licence',
        DocumentKind.taxCertificate => 'Tax registration',
        DocumentKind.companyPhoto => 'Workplace photo',
      };
}
