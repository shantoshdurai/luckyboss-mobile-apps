...
/// Brings the next question into view after an answer.
///
/// Shantosh: *"if they click something it automatically gives an animation and
/// then moves towards something like below, so that they don't want to scroll
/// manually and it will make the process very fast and effective."* He is right
/// about the cost — picking an industry on the employer wizard revealed "Which
/// job?" below the fold, so every posting involved a scroll to find a question
/// that had just appeared.
///
/// Two rules this deliberately follows:
///
/// * **Single-select only.** Scrolling after a multi-select tap fights someone
///   who has not finished choosing.
/// * **Never advances the whole step.** Moving to the next screen on one tap
///   makes a mis-tap unrecoverable without pressing Back.
///
/// The delay lets [RevealedField]'s [AnimatedSize] finish expanding — measuring
/// before that lands scrolls to where the field was about to be, not where it
/// ends up.
Future<void> revealNextQuestion(GlobalKey key) async {
  await Future<void>.delayed(const Duration(milliseconds: 160));
  // Read the context *after* the delay, not before: the field may not have
  // existed when this was called, and one that has since been disposed gives a
  // null here rather than a stale context to scroll.
  final revealed = key.currentContext;
  if (revealed == null || !revealed.mounted) return;
  await Scrollable.ensureVisible(
    revealed,
    duration: const Duration(milliseconds: 340),
    curve: Curves.easeInOutCubic,
    // A little above centre: the question sits high enough that its answers are
